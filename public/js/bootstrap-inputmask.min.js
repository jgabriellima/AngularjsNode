 ...
 */! function (a)
{
    ...
    {
        if (c) return;
        this.$element = a(b), this.mask = String(d.mask), this.options = a.extend(
        {}, a.fn.inputmask.defaults, d), this.init(), this.listen(), this.checkVal()
    };
    d.prototype = {
        init: function ()
        {
            var b = a.mask.definitions,
                c = this.mask.length;
        ...
        shiftL: function (a, b)
        {
            ...
            for (var d = a, e = this.seekNext(b); d < c; d++) if (this.tests[d])
                {
                    if (e < c && this.tests[d].test(this.buffer[e])) this.buffer[d] = this.buffer[e], this.buffer[e] = this.options.placeholder;
                    else break;
                    e = this.seekNext(e)
                }
            ...
            for (var c = a, d = this.options.placeholder; c < b; c++) if (this.tests[c])
                {
                    var e = this.seekNext(c),
                        f = this.buffer[c];
                    this.buffer[c] = d;
                    if (e < b && this.tests[e].test(f)) d = f;
                    else break
                }
        },
        unmask: function ()
        ...
        checkVal: function (a)
        {
            ...
            if (!a && d + 1 < this.partialPosition) this.$element.val(""), this.clearBuffer(0, b);
            else if (a || d + 1 >= this.partialPosition) this.writeBuffer(), a || this.$element.val(this.$element.val().substring(0, d + 1));
    ...
    }, a.fn.inputmask.defaults = {
        placeholder: "_"
    }, a.fn.inputmask.Constructor = d, a(function ()
    {
